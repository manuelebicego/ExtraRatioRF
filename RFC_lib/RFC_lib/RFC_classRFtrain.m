function rfc = RFC_classRFtrain(Xdata,lab,param)
% rfc = RFC_classRFtrain(Xdata,lab,param)
%  train forest with binary class tree with sampled negative points
%
...
while (i<=param.ntrees)
 % step 1: random sampling of a subset
    ...
    btx = Xdata(bt,:);
    btlab = lab(bt);
    if length(unique(lab))==length(unique(btlab))
        ...
        idn = 1;
        t1 = RFC_classtreetrain(btx,btlab,param,1);
        rfc.trees{i} = t1;
